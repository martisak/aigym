...
class Agent:
    ...
    def __init__(self, env):
        ...
        self.logger.debug(self.env.observation_space.high)
        ...
        self.interesting_states.append(self.prep_input(self.env.reset(), 8))
    ...
    def interesting_states_action(self):
        for s in self.interesting_states:
            p = self.model.predict(self.prep_input(s, 8), batch_size=1)
            yield p[0]
    ...
    def replay(self, batch_size):
        ...
        X = np.zeros((len(batches), 8))
        y = np.zeros((len(batches), 4))
...
def run_episode(env, agent, replay=32, max_timesteps=1000):
    ...
    observation = agent.prep_input(env.reset(), 8)
...
if __name__ == "__main__":
    ...
    logger.info("Making LunarLander-v2 environment")
    ...
    initial_state = np.zeros((episodes, 4))
    end_state = np.zeros((episodes, 4))

    for i_episode in range(episodes):

        t, eps = run_episode(env, agent, replay=64)
